...
def subset_select_ipot(X, Y, mu, gamma, L, max_outer_iter, max_inner_iter = 20 , wd = 2, disp_iter = False, return_map = False):
    '''
    ...
    def objective(alpha, beta, gamma, mu):
        ...
        temp2 = np.sum(alpha*mu) + np.sum(beta)/L
        return temp1 + temp2
    ...
    for outer_iter in range(0, max_outer_iter):
        ...
        for inner_iter in range(0, max_inner_iter):
            ...
            grad_y = 1/L - np.exp(-y/gamma) * (Q.T @ np.exp(-alpha/gamma))   
