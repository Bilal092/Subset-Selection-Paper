...
def pu_w_emd(p, q, C, nb_dummies=1):
    ...
    def df(G):
        ...
        for i in range(G.shape[1]):
            for lab in lstlab:
                temp = G[labels_a == lab, i]
                W[labels_a == lab, i] = temp*(np.linalg.norm(temp, 1))**(-0.5)
        return W
...
def compute_perf_emd(dataset_p, dataset_u, n_pos, n_unl, prior, nb_reps,  nb_dummies=1):
    ...
    for i in range(nb_reps):
        ...
        # transp_emd_group_list.append(transp_emd_group)
        y_hat = np.ones(len(y_u))
        sum_dummies = np.sum(transp_emd_group[-nb_dummies:], axis=0)
        nu_star_list.append(np.sum(transp_emd_group[0:-nb_dummies], axis=0))
        y_hat[np.argsort(sum_dummies)[nb_unl_pos:]] = 0
    ...
    return perfs, perfs_list, y_u_list, y_hat_list, nu_star_list
...
def compute_perf_pgw(dataset_p, dataset_u, n_pos, n_unl, prior, nb_reps,
    ...
    for i in range(nb_reps):
        ...
        for init in Ginit:
            transp_emd, t_loss = pu_gw_emd(C1, C2, mu, nu, nb_dummies, group_constraints=False, G0=init, log=True)
            y_hat = np.ones(len(y_u))
            sum_dummies = np.sum(transp_emd[-nb_dummies:], axis=0)
            y_hat[np.argsort(sum_dummies)[nb_unl_pos:]] = 0
        ...
        transp_emd_group = pu_gw_emd(C1, C2, mu, nu, nb_dummies,group_constraints=True, G0=None)
        # transp_emd_group_list.append(transp_emd_group)
        y_hat = np.ones(len(y_u))
        sum_dummies = np.sum(transp_emd_group[-nb_dummies:], axis=0)
        nu_star_list.append(np.sum(transp_emd_group[0:-nb_dummies], axis=0))
