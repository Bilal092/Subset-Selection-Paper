...
#%%
def subset_select_non_uniform_FISTA(mu, nu, c, K, M, gamma=1e-1, Lt=1/1e-1, eta = 1.1, max_iter = 20000, back_tracking = False, disp_iter=False):
    '''
    ...
    for k in range(0, max_iter):
        ...
        # bidual gradient 
        grad_y = c*nu - np.exp(-y/gamma) * (K.T @ np.exp(-alpha/gamma))   
        ...
        if back_tracking == True:
            ...
            z = y - 1/L_prev * grad_y
            z[z < 0] = 0
            while objective(alpha, z, gamma, mu, nu, c) > (fy + (grad_y.T@(z - y) + L_prev/2 * np.sum((z-y)**2)).item()):
                L_prev = L_prev * eta
                z = y - 1/L_prev * grad_y
                z[z < 0] = 0
            Lt = L_prev
